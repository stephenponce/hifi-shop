  ...
  module Shipping
    ...
    class FedEx < Carrier
      ...
      def find_rates(origin, destination, packages, options = {})
        ...
        rate_request = build_rate_request(origin, destination, packages)
...
## TODO: Add InsuredValue to the Web Service Request
## TODO: figure out if currency is req'd          iv << XmlNode.new('Currency', 'USA')

            packages.each do |pkg|
              rs << XmlNode.new('RequestedPackages') do |rps|
                rps << XmlNode.new('InsuredValue') do |iv|
		  iv << XmlNode.new('Currency', "USD")
                  iv << XmlNode.new('Amount', pkg.value)
                end
	        ...
	        ## Adding SpecialServicesRequested to web service request
                rps << XmlNode.new('SpecialServicesRequested') do |tw|
                  tw << XmlNode.new('SignatureOptionDetail') do |uz|
                    uz << XmlNode.new('OptionType', 'DIRECT')
                  end
                end
              end
            end
      ...
      def build_request_header
        ...
        trasaction_detail = XmlNode.new('TransactionDetail') do |td|
          td << XmlNode.new('CustomerTransactionId', 'AudiophileOasis') # TODO: Need to do something better with this..
        end
      ...
      def build_location_node(name, location)
	    ...
	    ## Setting Residential to be True (req'd for GRound Home Delivery Rates) -            
	    if(name == "Recipient")
	      address_node << XmlNode.new('Residential', true)
            end
## Support for Street Lines 
