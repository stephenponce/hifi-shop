...
class Calculator::ActiveShipping < Calculator
  ...
  def compute(line_items)
    ...
    destination = Location.new(:address1 => addr.address1,  
                              :address2 => addr.address2,
			      :country => addr.country.iso,
                              :state => (addr.state ? addr.state.abbr : addr.state_name),
                              :city => addr.city,
                              :zip => addr.zipcode)
## TODO - turn the cache back on
#    rates = Rails.cache.fetch(cache_key(line_items)) do
     rates = retrieve_rates(origin, destination, packages(order)) #TODO - use package price to calculate FedEx Rates (req'd for Insurance)
...
#    end
  ...
  def packages(order)
    ...
    value = order.line_items.inject(0) do |value, line_item|
      value + (line_item.total ? line_item.total : 0 )
    end
    package = Package.new(weight, [], :units => Spree::ActiveShipping::Config[:units].to_sym, :value => value)
    [package]
  end
